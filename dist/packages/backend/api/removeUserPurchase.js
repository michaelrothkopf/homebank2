...
module.exports = {
    ...
    route: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (!(yield verifyAuth_1.userCanAccessRecord(validationResult.user.id, req.body.purchaseId, record_1.RecordType.Purchase))) {
            res.send({
                failed: true,
                data: null
            });
        }
        const data = yield purchase_1.removeUserPurchase(req.body.purchaseId);
        res.send({
            failed: false,
            data: data,
        });
